...
// STATIC PATHS AND PROPS FOR MEDIA-ROOM ROUTES

export async function getStaticPropsMediaHomePage(
	client,
	categoryType,
	mediaType,
	page
) {
	const categories = await client.getAllByType(categoryType)
	const media = await client.getByType(mediaType, {
		pageSize: 9,
		page: page || 1,
		orderings: {
			field: 'document.first_publication_date',
...
export async function getStaticPropsMediaCategoryPage(
	client,
	categoryType,
	mediaType,
	categorySlug,
	...
	const categoryId = getCategoryIdUsingSlug.id
	const media = await client.getByType(mediaType, {
		filters: [prismic.filter.at(`my.${mediaType}.category`, categoryId)],
		pageSize: 9,
		page: page || 1,
		orderings: {
			field: 'document.first_publication_date',
